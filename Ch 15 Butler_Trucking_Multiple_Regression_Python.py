...
butler_trucking = pd.read_excel("C:\\Users\\16692\\Documents\\194 a fall 2021\\(5) R files\\Ch 15 Butler_Trucking_Multiple_Regression.xlsx")
...
pd.plotting.scatter_matrix(butler_trucking[variables],alpha=1,color="black") 
...
#the ~ sign with ols automatically adds the intercept for you, similar to adding a column of all ones
# but then running just ols(y,x)

#this is needed if you believe that if all predictors were equal to zero, the
# y value would not equal zero y. If yo dont want an intercept you do: ols(yvalue ~ 0 + predictors, data=df).fit()
